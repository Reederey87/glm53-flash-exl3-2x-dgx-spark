...
| **D1: conditional post-W28 checkpoint re-window** | W19 incumbent `7d74cdd` → previously rejected `bf582e4`, k=7/TP1 unchanged | **CLOSED for the compatible new checkpoint:** task 28 (2026-09-09) A/B'd `cfontes` TR3-v3 `45f7e139` vs `7d74cdd` and **REVERTED** (hashmap −2.2%, essay −3.5%, structured/prefill wash). Keep `7d74cdd`. Do not re-run TR3-v3 or `bf582e4` (W19). Do not chain k=2. Re-open D1 only for a later distinct compatible pin with new evidence. |
...
W19's original drafter pin; task 28 later rejected the compatible
`cfontes` TR3-v3 as a wash, so keep `7d74cdd` until a distinct new pin
appears. For larger prose gains, selective byte reduction has a real
artifact basis but higher quality/licensing cost. Another generic
fat-GEMM rewrite or EPLB flag is not the first move.
