...
# Gate v2 (kit PR #80, W23): cached follow-ups (uncached remainder <= WARM_TOKENS) bypass
# `skip`; held cold prefills proceed after MAX_WAIT_MS under LATE_CAP tokens/step
# (MAX_WAIT_MS=0 = wait forever = v1 behaviour).
...
validate_numeric_config() {
    ...
    # mixed-prefill gate v2 knobs (self-defaulting; 0 = feature off for the first two)
    GLM53_MIXED_PREFILL_WARM_TOKENS="${GLM53_MIXED_PREFILL_WARM_TOKENS:-3584}"
    GLM53_MIXED_PREFILL_MAX_WAIT_MS="${GLM53_MIXED_PREFILL_MAX_WAIT_MS:-1500}"
    GLM53_MIXED_PREFILL_LATE_CAP="${GLM53_MIXED_PREFILL_LATE_CAP:-512}"
    local _k _v _max
...
launch_cluster() {
    ...
    local -a nccl_common=(
        ...
        # workspace. Point it inside the already-bind-mounted vLLM cache so
        # it survives docker rm on both nodes (watchdog heals pay no re-JIT).
        -e FLASHINFER_WORKSPACE_BASE=/root/.cache/vllm/flashinfer
...
on_ready() {
    ...
    # LOCAL: no LAN URL - the API is bound to loopback on purpose (see --host above).
    log "  endpoints  : http://127.0.0.1:${PORT}/v1   (loopback only; reach it via SSH port-forward)"
