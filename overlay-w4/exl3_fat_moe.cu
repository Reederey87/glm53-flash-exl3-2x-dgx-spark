...
//
// Down mainloop: TILE_M rows x 128 columns per CTA, K advanced 32 per
// pipeline stage with cp.async multi-buffering (W3 zero-fill A-pad).
// Gate/up (W4): K advanced 128 per stage; the A-tile is fused gather +
// fp16 SUH + fp32 Hadamard from x[token[row]], so h13 is not allocated.
// B tiles (16x16 K4/MCG trellis tiles, 64 int16 words each) are staged
// through shared memory and dequantized in registers by the warp that
// owns that 16-column block, once per 16 K, and reused across all M
// blocks of the tile. The gate/up kernel runs two B streams (gate, up)
// for the same 128 intermediate columns so the SwiGLU and the
// down-projection input Hadamard fuse into its epilogue.
...
constexpr int FM_TILE_K = 32;                   // down pipeline stage
constexpr int FM_STAGES = 4;                    // down cp.async stages
constexpr int FM_FUSED_TILE_K = 128;            // gate/up A-tile / Hadamard width
constexpr int FM_FUSED_STAGES = 2;              // 4-stage 128-K does not fit SMEM
constexpr int FM_FUSED_K16 = FM_FUSED_TILE_K / 16;
...
template <int MB, int NS>
constexpr int fm_smem_bytes_fused()
{
    constexpr int a_stage = MB * 16 * FM_FUSED_TILE_K * 2;
    constexpr int b_stage = NS * FM_FUSED_K16 * FM_WARPS * FM_PACKED_WORDS * 2;
    constexpr int pipe = FM_FUSED_STAGES * (a_stage + b_stage);
    constexpr int epi = 16 * NS * FM_TILE_N * 4;
    return pipe > epi ? pipe : epi;
}

static_assert(fm_smem_bytes<FM_MB_DOWN, 2>() <= 101376, "down SMEM exceeds GB10 CTA cap");
static_assert(fm_smem_bytes_fused<FM_MB_GATEUP, 2>() <= 101376, "fused gateup SMEM exceeds GB10 CTA cap");
...
// ---------------------------------------------------------------------------
// Shared mainloop (down): acc[NS][MB][2] += A(tile rows, K) @ B_ns(K, 128 cols)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Fused gather + gate/up mainloop: A-tile is had128(x[token] * suh)
// ---------------------------------------------------------------------------

template <int MB, int NS, int NB_STRIDE>
__device__ __forceinline__ void fm_mainloop_fused(
    const half* __restrict__ x,          // [tokens, size_k]
    const int64_t* __restrict__ row_token,
    const half* __restrict__ suh,        // expert SUH [size_k]
    int size_k,
    int row0,
    int rows,
    const uint16_t* const (&packed)[NS],
    int tiles_n,
    int n_block0,
    half* sh_a,
    uint16_t* sh_b,
...
{
    constexpr int TILE_M = MB * 16;
    constexpr int TILE_K = FM_FUSED_TILE_K;
    constexpr int STAGES = FM_FUSED_STAGES;
    constexpr int K16 = FM_FUSED_K16;
    constexpr int A_STAGE = TILE_M * TILE_K;
    constexpr int B_STAGE = NS * K16 * FM_WARPS * FM_PACKED_WORDS;
    constexpr int B_CHUNKS = NS * K16 * FM_WARPS * 8;
    constexpr int B_ITERS = (B_CHUNKS + FM_THREADS - 1) / FM_THREADS;
    constexpr int ROW_ITERS = TILE_M / FM_WARPS;
    ...
    auto load_stage = [&](int stage, int kt)
    {
        ...
        for (int i = 0; i < B_ITERS; ++i)
        {
            ...
            if (c < B_CHUNKS)
            {
                int s = c / (K16 * FM_WARPS * 8);
                int r = c % (K16 * FM_WARPS * 8);
                int j = r / (FM_WARPS * 8);
                int nb = (r / 8) % FM_WARPS;
                int q = r % 8;
                const uint16_t* src = packed[s]
                    + ((int64_t) (kt * K16 + j) * tiles_n + n_block0 + s * NB_STRIDE + nb) * FM_PACKED_WORDS + q * 8;
                uint16_t* dst = sb + (s * K16 + j) * (FM_WARPS * FM_PACKED_WORDS) + nb * FM_PACKED_WORDS + q * 8;
                cp_async(dst, src);
            }
        }
        ...
        half* sa = sh_a + stage * A_STAGE;
        #pragma unroll
        for (int r_it = 0; r_it < ROW_ITERS; ++r_it)
        {
            int row = warp + r_it * FM_WARPS;
            half* row_smem = sa + row * TILE_K;
            int chunk = lane >> 1;
            int window = chunk >> 2;
            int local = chunk & 3;
            int sub = lane & 1;
            half* dst = row_smem + window * FM_TILE_K + fm_swz(row, local) * 8 + sub * 4;
            if (row < rows)
            {
                const int64_t token = row_token[row0 + row];
                const half* src = x + token * (int64_t) size_k + kt * TILE_K;
                const half* sh = suh + kt * TILE_K;
                // E2 boundary (had_hf_r_128_inner<pre_scale>): the input
                // scale is a fp16 x fp16 multiply, rounded, BEFORE the
                // fp32 Hadamard.
                half4 hv = *reinterpret_cast<const half4*>(src + lane * 4);
                half4 hs = *reinterpret_cast<const half4*>(sh + lane * 4);
            ...
            {
                fm_store_half4(dst, make_float4(0.f, 0.f, 0.f, 0.f));
            }
        }
    };

    if (0 < k_tiles) load_stage(0, 0);
    cp_async_fence();
    ...
    for (int kt = 0; kt < k_tiles; ++kt)
    {
        ...
        __syncthreads();
        int nk = kt + STAGES - 1;
        if (nk < k_tiles) load_stage(nk % STAGES, nk);
        cp_async_fence();
        ...
        for (int j32 = 0; j32 < 4; ++j32)
        {
            ...
            for (int j = 0; j < 2; ++j)
            {
                const int j16 = j32 * 2 + j;
                FragB fb[NS][2];
                #pragma unroll
                for (int s = 0; s < NS; ++s)
                {
                    const uint32_t* wb = reinterpret_cast<const uint32_t*>(
                        sb + (s * K16 + j16) * (FM_WARPS * FM_PACKED_WORDS) + warp * FM_PACKED_WORDS);
                    dq_dispatch<4, 1>(wb, lane << 3, fb[s][0], fb[s][1]);
                }
                #pragma unroll
                for (int mb = 0; mb < MB; ++mb)
                {
                    FragA fa;
                    int row = mb * 16 + a_row;
                    int local = j * 2 + a_chunk_hi;
                    ldsm4(fa, sa + row * TILE_K + j32 * FM_TILE_K + fm_swz(row, local) * 8);
                    #pragma unroll
                    for (int s = 0; s < NS; ++s)
                    {
                        ptx_mma_m16n8k16(fa, fb[s][0], acc[s][mb][0]);
...
// ---------------------------------------------------------------------------
// gate/up GEMM + SwiGLU + down-input Hadamard (W4 fused gather A-tile)
// ---------------------------------------------------------------------------
    ...
    float act_limit)
{
    ...
    uint16_t* sh_b = reinterpret_cast<uint16_t*>(
        sh_a + FM_FUSED_STAGES * MB * 16 * FM_FUSED_TILE_K);
    ...
    for (int seg = blockIdx.y; seg < num_segs; seg += gridDim.y)
    {
        ...
        const half* suh = suh_ptrs[e];
        const half* svh_g = gate_svh_ptrs[e] + n_base;
        const half* svh_u = up_svh_ptrs[e] + n_base;
        const half* suh_d = down_suh_ptrs[e] + n_base;

        FragC acc[NS][MB][2];
        fm_mainloop_fused<MB, NS, 0>(
            x, row_token, suh, size_k, row0, rows, packed, tiles_n,
            n_base / 16, sh_a, sh_b, acc);
    ...
    double act_limit)
{
    ...
    TORCH_CHECK(size_k % FM_FUSED_TILE_K == 0 && size_k >= FM_FUSED_TILE_K,
    ...
    constexpr int smem = fm_smem_bytes_fused<FM_MB_GATEUP, 2>();
    if (!fm_attr_set[0])
    {
